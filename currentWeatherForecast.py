...
def update_weather(screen_w, screen_h, font, fontcolor, font_height):
    ...
    screen.fill((0, 0, 0))
    weatherScreen = pygame.Surface((screen_w, int(screen_h)))
    ...
    if response.status_code == 200:
        ...
        # img = img.convert()
        screen.blit(img, (screen_w/2, -screen_h/10))
        ...
        try:
            ...
            pass
        try:
            rain = font.render(f"opady deszczu: {int(main['rain'])} mm/m²", True, fontcolor)
        except:
            pass
        try:
            snow = font.render(f"opady śniegu: {int(main['snow'])} mm/m²", True, fontcolor)
        except:
            pass

        # blit on weather screen
        from_left_border = screen_w/2.5
